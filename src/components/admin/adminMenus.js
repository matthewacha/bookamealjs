...
export class MenuList extends Component {
  ...
  DisplayMenus = (menuMeals, activeMenu) => {
    if (menuMeals.message) {
      return (
        <tbody>
          <tr>
            <td>
              <div className="panel-body"> Start by selecting a menu...</div>
            </td>
          </tr>
        </tbody>
      );
    ...
    if (menuMeals.Menu) {
      if (menuMeals.Menu[0].warning) {
        return (
          <tbody>
            <tr>
              <td>
                <div className="panel-body"> {menuMeals.Menu.warning}</div>
              </td>
            </tr>
          </tbody>
        );
        ...
        return (
          <tbody>
            {menuMeals.Menu.map((menuMeal, index) => {
      ...
      activeMenu.Menu.map((menuMeal, index) => {
        if (menuMeal.mealId !== null) {
          return (
            <tbody>
              <MenuView menuMeal={menuMeal} key={index} />
            </tbody>
          );
        }
      });
      ...
      <div>
        ...
        <div className="panel-heading">
          <form className="form-inline" onSubmit={this.AddMenuName}>
            ...
            <button className="btn btn-default" id="add-menu" type="submit">
              Add
            </button>
          </form>
        </div>
