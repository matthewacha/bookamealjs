...
export class MenuView extends Component {
  ...
  actualMealName = mealId => {
    ...
    if (menuMeals.Meals === undefined) {
      ...
      let meal,
        result = menuMeals.Meals.map(menuMeal => {
          if (mealId === menuMeal.id) {
            return menuMeal;
          }
        });
      for (meal of result) {
  ...
  actualMealPrice = mealId => {
    ...
    if (menuMeals.Meals === undefined) {
      ...
      let meal,
        result = menuMeals.Meals.map((menuMeal, index) => {
          if (mealId === menuMeal.id) {
            return menuMeal;
          }
        });
      ...
      <tr>
        ...
        <td>
          <div className="flex-container">
            <div id="delete-button" style={{ padding: "10px 5px" }}>
              <button
...
export default connect(
  mapStateToProps,
  { GetMeal, GetMenu, DeleteMenuMeal }
)(MenuView);
